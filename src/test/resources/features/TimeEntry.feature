...
Feature: Time entry
  ...
  Scenario Outline: Add a new time entry
    ...
    Examples:
      | description                    | hours | minutes | seconds | nameWorkspace | nameUser      | nameProject  | operation | entity     | jsonName        | statusCode |
      | Actualizacion de descripcion 3 | 02    | 00      | 00      | Crowdar       | brianazcona25 | pruebaManual | POST      | TIME_ENTRY | addNewTimeEntry | 201        |
  
  @EditFieldTimeRecord
  Scenario Outline: Edit a field in some time record
    Given An account created in Clockify and x-api-key generated
    And I perform a 'GET' to 'WORKSPACE' endpoint with the 'getAllWorkspaces' and ''
    And I get the workspaceId with name '<nameWorkspace>'
    And I perform a 'GET' to 'USER' endpoint with the 'getAllUserWorkspace' and ''
    ...
    Examples:
      | description                    | nameWorkspace | nameUser      | nameProject  | operation | entity     | jsonName             | statusCode |
      | Actualizacion de descripcion 2 | Crowdar       | brianazcona25 | pruebaManual | PUT       | TIME_ENTRY | updateFieldTimeEntry | 200        |
